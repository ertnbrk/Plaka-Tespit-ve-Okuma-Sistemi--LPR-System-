class AdminController {
    ...
    renderTable(searchTerm = '') {
        ...
        tbody.innerHTML = filtered.map(c => `
            <tr class="hover:bg-[#1E3A5F]/50 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap text-gray-300 font-mono text-xs">${c.date}</td>
    ...
    // Modal Actions
    openModal(id) {
        const complaint = window.api.getComplaint(id);
        if(!complaint) return;
        ...
        noteInput.value = complaint.adminNote || '';
        ...
        // Always show buttons to allow updating status (even if already refined)
    ...
    updateStatusFromModal(newStatus) {
        if(this.currentId) {
            ...
            // Auto-fill note if empty so the user sees a message
            if (!note) {
    ...
    performUpdate(id, newStatus, note) {
        window.api.updateComplaint(id, { status: newStatus, adminNote: note });
        this.renderTable(document.getElementById('searchPlate').value);
        this.renderStats();
    }
}
